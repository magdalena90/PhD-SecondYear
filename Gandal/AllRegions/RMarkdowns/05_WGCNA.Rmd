...
# Saving the degree of the nodes for future analysis
node_degree = data.frame('ID' = rownames(TOM), 'degree' = rowSums(TOM))
write.csv(node_degree, file = './../Data/node_degree.csv', row.names = FALSE)

# library(ggpubr)
plot_data = node_degree %>% left_join(DE_info, by ='ID') %>%
            mutate(gene.score = ifelse(gene.score=='None','Others',gene.score))
...
p = plot_data %>% ggplot(aes(gene.score, degree, fill = gene.score)) +
              ...
              stat_compare_means(comparisons = comparisons, label = 'p.signif', method = 't.test',
                                 ...
                                 tip.length = .02) + scale_fill_manual(values=SFARI_colour_hue(r=c(1:3,8,7))) +
              xlab('SFARI Scores') + ylab('Weighted Degree in Network') +
              theme_minimal()+ theme(legend.position='none')
load('./../Data/Ridge_model.RData')
plot_data = node_degree %>% inner_join(predictions, by = 'ID') %>% inner_join(data.frame('ID'=rownames(datExpr),'meanExpr'=rowMeans(datExpr)))
plot_data %>% ggplot(aes(degree,prob)) + geom_point(alpha=0.2, color = '#0099cc') + geom_smooth(color = 'gray', alpha=0.2) + theme_minimal()
plot_data %>% ggplot(aes(meanExpr,prob)) + geom_point(alpha=0.2, color = '#0099cc') + geom_smooth(color = 'gray', alpha=0.2) + theme_minimal()
...
A lot of genes close to the root of the dendrogram remain unclassified
