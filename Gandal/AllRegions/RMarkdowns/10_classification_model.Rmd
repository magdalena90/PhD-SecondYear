...
dataset = dataset %>% mutate(Module = clusterings$Module) %>% dplyr::select(-matches(clustering_selected))
...
rm(getinfo, mart, rownames_dataset, GO_annotations, g, GS_missing, dds)
```
<br>
...
new_dataset = dataset %>% filter(Module != 'gray' & !is.na(MTcor)) %>% 
              ...
              mutate('absGS'=abs(GS), 'SFARI'=gene.score!='Others') %>% dplyr::select(-gene.score)
...
plot_data = test_set %>% mutate(ID=rownames(test_set)) %>% dplyr::select(ID, prob) %>%
            ...
            mutate(gene.score = ifelse(gene.score=='Others', ifelse(ID %in% GO_neuronal$ID, 'Neuronal', 'Others'), 
                                       gene.score)) %>%
...
plot_data %>% ggplot(aes(gene.score, prob)) + 
...
negative_set %>% dplyr::select(prob, SFARI) %>% mutate(ID = rownames(negative_set)) %>% 
                 arrange(desc(prob)) %>% top_n(50, wt = prob) %>%
                 left_join(original_dataset %>% mutate(ID = rownames(original_dataset)), by = 'ID')  %>%
                 mutate(gene.score = ifelse(gene.score=='Others', 
...
plot_data %>% ggplot(aes(meanExpr, prob)) + geom_point(alpha=0.2, color='#0099cc') + 
              geom_smooth(method='loess', color='gray', alpha=0.2) + xlab('Mean Expression') + 
...
plot_data %>% ggplot(aes(log2FoldChange, prob)) + geom_point(alpha=0.1, color='#0099cc') + 
              geom_smooth(method='loess', color='gray', alpha=0.2) + xlab('LFC') +
