...
library(tidyverse)
library(knitr)
library(plotly) ; library(viridis) ; library(gridExtra) ; library(RColorBrewer) ; library(ggpubr) ; library(rstatix)
library(reshape2)
library(mgcv) # GAM
...
wt = plot_data %>% wilcox_test(corrected_prob~gene.score, p.adjust.method='BH') %>% add_x_position(x='group')

increase = 0.07
base = 0.75
pos_y_comparisons = c(base + c(0,1,3,5)*increase, base+c(0,2,4)*increase, base+0:1*increase, base)

plot_data %>% filter(!is.na(gene.score)) %>% ggplot(aes(gene.score, corrected_prob)) + 
