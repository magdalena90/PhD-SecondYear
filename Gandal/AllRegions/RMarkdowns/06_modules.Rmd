...
Dynamic Tree leaves many more genes without a cluster. As we saw in the dendrograms in 05_WGCNA.html, the genes usually left without a cluster are the ones closest to the root of the dendrogram, now we want to see if they share some characteristic in common: we suspect it may be a weak relation to ASD.
...
The Dynamic Hybrid algorithm created `r length(unique(genes_info$Module))-1` modules and leaves `r sum(genes_info$Module=='gray')` genes without a module
...
It's a good sign that the gray module has a small correlation with diagnosis, since we know its composed mainly of not differentially expressed genes.
...
Log Fold Chance has a wider value range with outliers in the extremes, but both variables agree with each other quite well
...
- All SFARI scores have a higher median than the rest of the genes without a neuronal-related annotation
...
- SFARI score 1 is the group with the lowest Module-Diagonsis correlation <span style="color:red">(!)</span>

- Most differences are not statistically significant, but SFARI Score 1 does have a significant difference with all the other grous
...
plot_data %>% ggplot(aes(gene.score, abs(MTcor), fill=gene.score)) + 
              geom_boxplot(outlier.colour='#cccccc', outlier.shape='o', outlier.size=3) + 
              stat_compare_means(comparisons = comparisons, label = 'p.signif', method = 't.test', 
                                 method.args = list(var.equal = FALSE), label.y = pos_y_comparisons, 
                                 tip.length = .01) +
...
- SFARI Score 1 has a lower median than the non-SFARI non-Neuronal genes <span style="color:red">(!)</span>
...
- The highest SFARI scores have a lower Module-Trait correlation <span style="color:red">(!)</span>
...
- With both SFARI Gene lists, there seems to be a small negative correlation between Mdoule-Trait correlation and SFARI Enrichment, with the genes with a negative Module-Diagnosis correlation having higher SFARI enrichment and genes with positive Module-Diagnosis correlation lower SFARI enrichment. This pattern is a bit stronger in the old SFARI Genes

- In the histogram on the right we can see that the enrichment for the new SFARI Genes is stronger for the new SFARI Genes, with more values closer to the extremes (0 and 1) and less enrichments closer to the middle
