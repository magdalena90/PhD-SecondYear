...
This plot shows that our above hypothesis was incorrect: In general, the higher the level of expression, the lower the LFC Magnitude, but this pattern flattens for the genes with the highest levels of expression of the whole dataset, which is the region where the SFARI Genes are. So based on their neighbourhood by level of expression, we would expect them to have similar LFC magnitudes (and perhaps slightly higher for SFARI Score 1, but definitely not lower)
...
aux_data = plot_data %>% filter(!gene.score %in% c('Neuronal','Others')) %>% group_by(gene.score) %>%
           dplyr::summarise(mean_by_score = mean(meanExpr)) %>%  mutate('color' = SFARI_colour_hue(r=1:6)[1:3])
...
- The difference with the Neuronal group is statistically significant for all SFARI Scores
...
create_plot_by_SFARI_score = function(score, n) {
  ...
  plot = std_magnitudes %>% ggplot(aes(gene.score, std_magnitude)) + 
         geom_boxplot(outlier.colour='#e6e6e6', outlier.shape='o', outlier.size=3, aes(fill=gene.score)) +
...
- All SFARI scores are more affected by the filtering than the genes with Neuronal annotations
