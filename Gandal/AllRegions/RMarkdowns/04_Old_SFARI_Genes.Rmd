...
`r sum(genes_info$gene.score %in% c('1','2','3','4','5','6') & genes_info$Neuronal)` of these genes have a SFARI score
```{r}
cro(table_info$gene.score[genes_info$`gene-score` %in% as.character(c(1:6))],
    list(table_info$Neuronal[genes_info$`gene-score` %in% as.character(c(1:6))], total()))
...
p1 = plot_data %>% ggplot(aes(Group, MeanExpr)) + 
...
wt = plot_data %>% wilcox_test(SDExpr~Group, p.adjust.method='BH') %>% add_x_position(x = 'group')
increase = 0.03
base = 0.38
pos_y_comparisons = c(base, base+increase, base)
p2 = plot_data %>% ggplot(aes(Group, SDExpr)) + 
     geom_boxplot(outlier.colour='#cccccc', outlier.shape='o', outlier.size=3, aes(fill=Group)) +
...
Smaller LFC magnitude than Neuronal genes and similar but slightly lower than the rest of the genes. The differences are statistically significant in all cases
...
base = 0.45
pos_y_comparisons = c(base, base + increase, base)
plot_data %>% ggplot(aes(Group, abs(log2FoldChange))) + 
     geom_boxplot(outlier.colour='#cccccc', outlier.shape='o', outlier.size=3, aes(fill=Group)) +
     stat_pvalue_manual(wt, y.position = pos_y_comparisons, tip.length = .01) +
     scale_fill_manual(values=c('#00A4F7', SFARI_colour_hue(r=c(8,7)))) + 
     coord_cartesian(ylim= c(0.05, max(pos_y_comparisons))) +
...
SFARI Genes have consistently a lower percentage of DE genes than the Genes with Neuronal annotations and a slightly hiher level to the rest of the genes
...
increase = 1
base = 15.5
pos_y_comparisons = c(base + c(0,1,3,6,10,13,15)*increase,    # 1
                      base + c(0,2,4,7,11,14)*increase,       # 2
                      base + c(0,1,5,8,12)*increase,          # 3
                      base + c(0,2,3,9)*increase,             # 4
                      base + c(0,1,4)*increase,               # 5
                      base + c(0,2)*increase,                 # 6
                      base)                                   # Neuronal
p1 = plot_data %>% ggplot(aes(gene.score, MeanExpr)) + 
...
wt = plot_data %>% wilcox_test(SDExpr~gene.score, p.adjust.method='BH') %>% add_x_position(x = 'group')
increase = 0.03
base = 0.43
pos_y_comparisons = c(base + c(0,1,3,6,10,13,15)*increase, base + c(0,2,4,7,11,14)*increase,
                      base + c(0,1,5,8,12)*increase, base + c(0,2,3,9)*increase,
                      base + c(0,1,4)*increase, base + c(0,2)*increase, base)
p2 = plot_data %>% ggplot(aes(gene.score, SDExpr)) + 
     geom_boxplot(outlier.colour='#cccccc', outlier.shape='o', outlier.size=3, aes(fill=gene.score)) +
...
rm(p1, p2, base, increase, pos_y_comparisons, wt)
```
...
```{r fig.align='center', fig.width=66, fig.height=7}
...
This plot shows that our above hypothesis was incorrect: In general, the higher the level of expression, the lower the LFC Magnitude, but this pattern flattens for the genes with the highest levels of expression of the whole dataset, which is the region where the SFARI Genes are except for the SFARI SCore 6. So based on their neighbourhood by level of expression, we would expect SFARI Genes with score 6 to have higher LFC manitudes and the rest of them to have similar ones (and perhaps slightly higher for SFARI Score 1, but definitely not lower)

```{r warning=FALSE, message=FALSE, fig.width=10}

plot_data = data.frame('meanExpr' = rowMeans(datExpr), 'LFC_magnitude' = abs(genes_info$log2FoldChange), 
                       'gene.score' = genes_info$gene.score, 'p' = NA, 'sd' = NA) %>% arrange(meanExpr)

w = 1000
for(i in 1:(nrow(plot_data)-w)){
  plot_data$p[i+floor(w/2)] = mean(plot_data$LFC_magnitude[i:(i+w)])
  plot_data$sd[i+floor(w/2)] = sd(plot_data$LFC_magnitude[i:(i+w)])
}

aux_data = plot_data %>% filter(!gene.score %in% c('Neuronal','Others')) %>% group_by(gene.score) %>%
           dplyr::summarise(mean_by_score = mean(meanExpr)) %>%  mutate('color' = SFARI_colour_hue(r=1:6)[1:6])

ggplotly(plot_data %>% filter(!is.na(p)) %>% ggplot(aes(meanExpr, p)) + geom_line() +
         geom_ribbon(aes(ymin=p-sd, ymax=p+sd), linetype=0, alpha=0.3, fill = 'gray') +
...
- The difference with the Neuronal group is statistically significant for all SFARI Scores except scores 5 and 6
...
create_plot_by_SFARI_score = function(score, n) {
  ...
  wt = ggpubr::compare_means(std_magnitude~gene.score, data=std_magnitudes, ref.group=score %>% as.character) %>% 
       add_x_position(x = 'group2')
  ...
  plot = std_magnitudes %>% ggplot(aes(gene.score, std_magnitude)) + 
         ...
         stat_pvalue_manual(wt, y.position = 3, remove.bracket=TRUE, xmin='group2', label = 'p.signif') +
         theme_minimal() + theme(legend.position = 'none')
...
p4 = create_plot_by_SFARI_score(4,n)
p5 = create_plot_by_SFARI_score(5,n)
p6 = create_plot_by_SFARI_score(6,n)
...
rm(p1, p2, p3, p4, p5, p6, get_std_lfc_magnitudes, create_plot_by_SFARI_score)
```
...
The proportion of DE genes for each SFARI Genes is quite similar, but it tends to be lower for the highest scores
...
But in our dataset, the higher the level of expression of a gene, the more likely the gene is to be DE (this can be seen by ordering the genes by level of expression and calculating the proportion of DE Genes using a sliding window). Based one this, the SFARI Score 1 should have the highest proportion of DE Genes, followed by the others and SFARI Score 6 should have the lowest proportion of all.
...
aux_data = plot_data %>% filter(!gene.score %in% c('Neuronal','Others')) %>% group_by(gene.score) %>%
           ...
           mutate('color' = SFARI_colour_hue(r=1:6))
...
rm(p1, p2, p3, p4, p5, p6, get_neighbours_DE, calc_dist, create_plot_by_SFARI_score, n)
```
...
- The higher the SFARI Score, the lower the % of DE Genes it contains

- All SFARI scores except score 6 are more affected by the filtering than the genes with Neuronal annotations

- Using the null hypothesis $H_0: lfc=0$, `r sum(genes_info$significant & !genes_info$gene.score %in% c('Neuronal','Others'))`/`r sum(!genes_info$gene.score %in% c('Neuronal','Others'))` SFARI genes are statistically significant (~`r round(100*sum(genes_info$significant & !genes_info$gene.score %in% c('Neuronal','Others')) / sum(!genes_info$gene.score %in% c('Neuronal','Others')))`%)
...
ggplotly(lfc_counts_all %>% filter(group != 'All') %>% ggplot(aes(lfc, perc, color=group)) + 
         geom_point(aes(id=n)) + geom_line() + scale_color_manual(values=SFARI_colour_hue(r=c(1:6,8,7))) + 
         ylab('% of Differentially Expressed Genes') +  xlab('Fold Change') + 
