...
```
There are `r sum(plot_data$distance>3)` genes with a difference between Diagnoses larger than 3 SD to the distance distribution of all genes. Gene `r plot_data$ID[which.max(plot_data$distance)] %>% as.character` has the largest difference in mean expression between ASD and CTL, but it has a low level of expression, so it probably won't be statistically significant (we confirm this in the Differential Expression Section)
...
- Only `r sum(DE_info$padj<0.05, na.rm=T)` genes (~`r round(mean(DE_info$padj<0.05, na.rm=T)*100)`% vs ~28% in Gandal’s dataset) are significant using a threshold of 0.05 for the adjusted p-value and a without a log Fold Change threshold (keeping the null hypothesis $H_0: LFC=0$)

- `r sum(DE_info$padj<0.05, na.rm=T)` genes (~`r round(mean(DE_info$padj<0.05, na.rm=T)*100)`%) are significant using a threshold of 0.05 for the adjusted p-value and a without a log Fold Change threshold (keeping the null hypothesis $H_0: lfc=0$)

- Some genes don't have an adjusted p-value (`r sum(is.na(DE_info$padj))`)
```{r DE_count}
...
- There is a negative relation between LFC and mean expression, for both differentially and not differentially expressed groups of genes
