...
genes_info = DE_info %>% mutate(ID = datGenes$ensembl_gene_id) %>% 
...
- Genes assigned to a cluster also seem to have a higher level of expression than the genes left unassigned (because they have higher PC1 values)
...
datTraits = datMeta %>% dplyr::select(Diagnosis, Brain_lobe) %>%
            dplyr::rename('Brain Lobe' = Brain_lobe)
...
- In general, modules have strong correlations with Diagnosis with really small p-values and not much relation with brain regions

```{r plot_heatmap, fig.width=5, fig.height=20, warning=FALSE, message=FALSE}
...
plot_data = dataset %>% dplyr::select(ID, MTcor, GS) %>% 
            ...
            left_join(genes_info %>% dplyr::select(ID,AveExpr,log2FoldChange,significant,Module), by='ID') %>%
...
ggplotly(plot_data %>% ggplot(aes(order, log2FoldChange, group=order)) + 
         geom_hline(yintercept=0, color='gray', linetype='dotted') +
...
ggplotly(plot_data %>% ggplot(aes(order, log2(AveExpr+1), group=order)) + 
         geom_hline(yintercept=mean(log2(plot_data$AveExpr+1)), color='gray', linetype='dotted') +
         geom_boxplot(fill=unique(plot_data$Module)) + theme_minimal() + 
...
plot_data %>% ggplot(aes(MTcor, log2(AveExpr+1))) + geom_point(alpha=.2, color=plot_data$Module, aes(id=ID)) + 
         geom_hline(yintercept=mean(log2(plot_data$AveExpr+1)), color='gray', linetype='dotted') + 
         geom_smooth(color='gray', alpha=0.3) + theme_minimal() + 
         xlab('Module-Diagnosis correlation') + ylab('log2(Mean Expression)') +
         ggtitle(paste0('R^2=',round(cor(plot_data$MTcor, log2(plot_data$AveExpr+1))^2,2)))
```
<br>
...
- SFARI Score 6 has the highest median of all groups of genes <span style="color:red">(!)</span>
...
- SFARI Scores 2 to 5 have similar distributions

- SFARI Score 6 has the highest Module-Diagonsis correlation magnitude distribution. It seems to have a distribution different to the rest of the SFARI genes

- SFARI Score 1 also has a high Module-Diagnosis correlation magnitude distribution
...
- With both SFARI Gene lists, there seems to be a correlation between Mdoule-Diagnosis correlation and SFARI Enrichment in modules with negative Module-Diagnosis correlation

- In the histogram on the right we can see that the enrichment for the new SFARI Genes is slightly weaker (with higher density close to zero) than the old SFARI Genes
